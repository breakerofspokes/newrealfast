...
const uint32_t CHRYSLER_PARAM_RAM_HD_S0 = 3U;  // set for Ram HD platform S0 Hardware
...
static int chrysler_fwd_hook(int bus_num, CANPacket_t *to_fwd) {
  ...
  const bool is_lkas_button = (chrysler_platform != CHRYSLER_PACIFICA) && ((addr == chrysler_addrs->CENTER_STACK_1) || (addr == chrysler_addrs->CENTER_STACK_2));
  
  // forward CAN 0 & 1 -> 2 so stock LKAS camera sees messages
  if (bus_num == 0){
    if ((chrysler_platform != CHRYSLER_RAM_HD_S0) && !is_ram_cruise){
      bus_fwd = 2;
    }
    else {
