...
class CarController():
  ...
  def lkas_control(self, CS, actuators, can_sends, enabled, hud_alert, jvepilot_state):
    ...
    #  self.moving_fast = True or low_steer_models
    if low_steer_models:
      self.moving_fast = not CS.out.steerError and CS.lkas_active# and (self.moving_fast or CS.torq_status > 1)
      ...
      if CS.out.vEgo > (CS.CP.minSteerSpeed - 0.5):  # for command high bit
        ...
        if CS.out.vEgo < (CS.CP.minSteerSpeed - 3.0):
          self.moving_fast= False  # < 14.5m/s stock turns off this bit, but fine down to 13.5

    if self.moving_fast_previous == True and self.moving_fast== False:
        self.lkaslast_frame = self.ccframe

    if (CS.out.steerError is True) or (self.ccframe-self.lkaslast_frame<400):#If the LKAS Control bit is toggled too fast it can create an LKAS error
      self.moving_fast = False

    lkas_active = self.moving_fast and enabled
    
    if not lkas_active or self.moving_fast_previous == False:
      apply_steer = 0

    self.apply_steer_last = apply_steer

    self.moving_fast_previous = self.moving_fast
    ...
    new_msg = create_lkas_command(self.packer, int(apply_steer), self.moving_fast, lkas_counter)
    can_sends.append(new_msg)
